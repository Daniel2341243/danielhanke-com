...
export async function generateMetadata({
  params,
}: PageProps<"/[locale]/coaching-online">): Promise<Metadata> {
  const { locale } = await params;
  return buildPageMetadata({
    locale: locale as Locale,
    pathname: "/coaching-online",
    titleKey: "coachingOnlineTitle",
    descriptionKey: "coachingOnlineDescription",
  });
}

export default async function CoachingOnlinePage({
  params,
}: PageProps<"/[locale]/coaching-online">) {
  const { locale } = await params;
  if (!hasLocale(routing.locales, locale)) notFound();
  setRequestLocale(locale);

  const t = await getTranslations("coachingOnline");
  const mailto = buildCoachingMailto({ format: "Coaching über Zoom" });
